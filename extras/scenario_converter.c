...
usage()
{
    ...
    fprintf(stderr, "\t -t : output type, text or binary(Default: binary)\n");
}
...
FILE *ofile_fd;
{
    ...
    fprintf(ofile_fd, "%% time from_id "
            "from_node_x from_node_y from_node_z "
            "to_id to_node_x to_node_y to_node_z "
            "distance Pr SNR FER num_retr op_rate "
            "bandwidth loss_rate delay jitter\n");
    ...
    for(time_i = 0; time_i < bin_hdr.time_record_number; time_i++) {
        ...
        for(rec_i = 0; rec_i < bin_time_rec.record_number; rec_i++) {
            ...
            fprintf(ofile_fd, "%.4f "
                ...
                recs[rec_i].bandwidth, recs[rec_i].loss_rate, recs[rec_i].delay, 0.000000);
        }
    }
...
char **argv;
{
    ...
    while((c = getopt(argc, argv, "hs:d:i:o:t:")) != -1) {
        switch(c) {
        case 'h':
            usage();
            exit(0);
