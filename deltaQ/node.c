...
// init a node
void
node_init(node, name, type, id, ssid, connection, position_x, position_y, position_z, internal_delay)
...
struct node_class *node;
{
    ...
    printf("  Node '%s': type='%s' id=%d ssid='%s' connection='%s'\n", 
            node->name, node_types[node->type], node->id, node->ssid, node_connections[node->connection]);
    printf("\tposition=(%.2f, %.2f, %.2f) if_num=%d\n",
            node->position.c[0], node->position.c[1], node->position.c[2], node->if_num);
...
void
node_copy(node_dst, node_src)
struct node_class *node_dst;
struct node_class *node_src;
{
    int i;
...
int
node_auto_connect(from_node, scenario)
struct node_class *from_node;
struct scenario_class *scenario;
{
    ...
    if(scenario->connection_number < MAX_CONNECTIONS) {
        ...
        for(i = 0; i < scenario->node_number; i++) {
            ...
            if(scenario->nodes[i].id != from_node->id) {
                ...
                if(scenario->nodes[i].type == ACCESS_POINT_NODE) {
                    ...
                    if((scenario->nodes[from_node_index].connection == INFRASTRUCTURE_CONNECTION) ||
                            (scenario->nodes[from_node_index].connection == ANY_CONNECTION)) {
                        //the "ssid" must be the same with that of the access point
                        if(strcmp (scenario->nodes[from_node_index].ssid, scenario->nodes[i].ssid) == 0) {
                            try_connect = TRUE;
                        }
                    }
                }
                // assume the potential to_node is a regular node
                    ...
                    if((scenario->nodes[from_node_index].connection == AD_HOC_CONNECTION) ||
                            (scenario->nodes[from_node_index].connection == ANY_CONNECTION)) {
        ...
        if(max_Pr_index != INVALID_INDEX) {
            ...
            strncpy(connection->from_node, scenario->nodes[from_node_index].name, MAX_STRING - 1);
            strncpy(connection->to_node, scenario->nodes[max_Pr_index].name, MAX_STRING - 1);
            strncpy(connection->through_environment, scenario->environments[environment_index].name, MAX_STRING - 1);
            ...
            WARNING ("No connection can be established for node '%s'", from_node->name);
        }
    }
    else {
        WARNING("Maximum number of connections in scenario reached (%d)", MAX_CONNECTIONS);
        return ERROR;
    }
...
// NOTE: This function is still in experimental phase!
int
node_auto_connect_at(from_node, scenario)
struct node_class *from_node;
struct scenario_class *scenario;
{
    int i;
    ...
    for(i = 0; i < scenario->node_number; i++) {
        ...
        if(scenario->nodes[i].id != from_node->id) {
            if(scenario->connection_number < MAX_CONNECTIONS) {
                DEBUG("Building a candidate connection from '%s' to '%s'", from_node->name, scenario->nodes[i].name);

                // use the last connection element during searching process
                // by creating a temporary connection
                candidate_connection = &(scenario->connections[scenario->connection_number]);

                // make sure all parameters are initialized to default values 
                if(connection_init (candidate_connection, DEFAULT_PARAMETER,
                            DEFAULT_PARAMETER, DEFAULT_PARAMETER, 7,
                ...
                candidate_connection->through_environment_index = environment_index;

                // update the connection, so that the fields is computed
                if(active_tag_connection_update (candidate_connection, scenario) == ERROR) {
                    return ERROR;
                }

                active_tag_loss_rate(candidate_connection, scenario, &(candidate_connection->loss_rate));
...
#endif
                    // finalize connection creation
                    strncpy(candidate_connection->from_node, scenario->nodes[from_node_index].name, MAX_STRING - 1);
                    strncpy(candidate_connection->to_node, scenario->nodes[i].name, MAX_STRING - 1);
                    strncpy(candidate_connection->through_environment,
                            scenario->environments[environment_index].name, MAX_STRING - 1);
    ...
    if(created_connections != 0) {
        DEBUG ("Auto-connect node '%s': created %d connections", from_node->name, created_connections);
    }
    else {
        DEBUG ("Auto-connect node '%s': no connection established", from_node->name);
    }
...
// return SUCCESS on succes, ERROR on error
int
node_init_interface_index(node, scenario)
struct node_class *node;
struct scenario_class *scenario;
{
    int i;
    ...
    DEBUG("Initialized %d interface(s) for node %d (global interface number=%d)",
