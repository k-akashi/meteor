 ...
 *
 * $Id: active_tag.c 146 2013-06-20 00:50:48Z razvan $
 *
 ***********************************************************************/
...
// Reference: 
struct parameters_active_tag s_node = {
...
void
active_tag_print_parameters(params)
struct parameters_active_tag *params;
{
...
active_tag_get_interface_adapter(interface)
struct interface_class *interface;
{
    struct parameters_active_tag *adapter;

    if(interface->adapter_type == S_NODE) {
        adapter = (&s_node);
    }
    else {
        WARNING("Defined active tag type %d is unknown", interface->adapter_type);
        return NULL;
    }
...
int
active_tag_fer(connection, scenario, operating_rate, fer)
    ...
    if(environment->num_segments > 0) {
        ...
        for(seg_i = 0; seg_i < environment->num_segments; seg_i++) {
            if(environment->W[seg_i] > 0) {
                (*fer) = 1.0;
                DEBUG("Adjusted connection FER = %f (building attenuation)", (*fer));
...
// return SUCCESS on succes, ERROR on error
int
active_tag_loss_rate(connection, scenario, loss_rate)
struct connection_class *connection;
struct scenario_class *scenario;
double *loss_rate;
{
    if(active_tag_fer(connection, scenario, connection->operating_rate, &(connection->frame_error_rate)) == ERROR) {
...
// return SUCCESS on succes, ERROR on error
int
active_tag_operating_rate(connection, scenario, operating_rate)
struct connection_class *connection;
struct scenario_class *scenario;
int *operating_rate;
{
...
// return SUCCESS on succes, ERROR on error
int
active_tag_delay_jitter(connection, scenario, variable_delay, delay, jitter)
struct connection_class *connection;
struct scenario_class *scenario;
...
// return SUCCESS on succes, ERROR on error
int
active_tag_bandwidth(connection, scenario, bandwidth)
struct connection_class *connection;
struct scenario_class *scenario;
double *bandwidth;
{
    ...
    adapter = active_tag_get_interface_adapter(&((scenario->nodes[connection->from_node_index]).interfaces[connection-> from_interface_index]));
...
// return SUCCESS on succes, ERROR on error
int
active_tag_connection_update(connection, scenario)
struct connection_class *connection;
struct scenario_class *scenario;
{
    // helper "shortcuts"
...
struct interface_class *interface;
{
    ...
    interface->Pr0 = interface->Pt - 20 * log10((4 * M_PI) / (SPEED_LIGHT / active_tag_frequencies[0])) +
...
// return SUCCESS on succes, ERROR on error
int
active_tag_compute_interference(connection, connection_i, scenario)
struct connection_class *connection;
...
struct scenario_class *scenario;
{
    ...
    active_tag_loss_rate(&virtual_connection, scenario, &(virtual_connection.loss_rate));
    INFO("Loss rate for virtual connection = %.3f", virtual_connection.loss_rate);

    // interference is considered in the form of additional 
    // error rate induced by other transmitters

    connection->interference_fer += (1.0 / (9 * 9) * (1 - virtual_connection.frame_error_rate));
...
// return SUCCESS on succes, ERROR on error
int
active_tag_interference(connection, scenario)
struct connection_class *connection;
struct scenario_class *scenario;
{
    int connection_i;
    ...
    for(connection_i = 0; connection_i < scenario->connection_number; connection_i++) {
        ...
        if(connection != &(scenario->connections[connection_i])) {
            ...
            if((connection->standard == ACTIVE_TAG) && (scenario->connections[connection_i].standard == ACTIVE_TAG)) {
                DEBUG("--------------------------------------------");
                DEBUG("Interference between active tags '%s' and '%s' detected \
                        => determine effects", connection->from_node, scenario->connections[connection_i].from_node);
                active_tag_compute_interference(connection, &(scenario->connections[connection_i]), scenario);
            }
        }
    }
