 ...
 *
 * $Id: io.c 161 2014-02-04 00:32:16Z razvan $
 *
 ***********************************************************************/
...
// return SUCCESS on succes, ERROR on error
int
io_write_ns2_motion_header_to_file_new (struct scenario_class *scenario,
					FILE * motion_file, float current_time)
{
  // write comment
  fprintf (motion_file, "# NS-2 node movement file generated by QOMET\n");
...
// return SUCCESS on succes, ERROR on error
int
io_write_ns2_motion_info_to_file_new (struct scenario_class *scenario,
				      FILE * motion_file, float time)
{
  int node_i;
  struct node_class *node;
  ...
  for (node_i = 0; node_i < scenario->node_number; node_i++)
    {
      node = &(scenario->nodes[node_i]);
      fprintf (motion_file, "$ns_ at %.3f $node_(%d) set X_ %.9f\n", time, node->id, node->position.c[0]);
      fprintf (motion_file, "$ns_ at %.3f $node_(%d) set Y_ %.9f\n", time, node->id, node->position.c[1]);
    }
