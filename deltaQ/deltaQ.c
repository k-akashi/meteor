...
license(FILE *f)
{
    fprintf(f, "Copyright (c) 2006-2013 The StarBED Project  All rights reserved.\n");
    fprintf(f, "\n");
    fprintf(f, "Redistribution and use in source and binary forms, with or without\n");
    fprintf(f, "modification, are permitted provided that the following conditions\n");
    fprintf(f, "are met:\n");
    fprintf(f, "1. Redistributions of source code must retain the above copyright\n");
    fprintf(f, "   notice, this list of conditions and the following disclaimer.\n");
    fprintf(f, "2. Redistributions in binary form must reproduce the above copyright\n");
    fprintf(f, "   notice, this list of conditions and the following disclaimer in the\n");
    fprintf(f, "   documentation and/or other materials provided with the distribution.\n");
    fprintf(f, "3. Neither the name of the project nor the names of its contributors\n");
    fprintf(f, "   may be used to endorse or promote products derived from this software\n");
    fprintf(f, "   without specific prior written permission.\n");
    fprintf(f, "\n");
    fprintf(f, "THIS SOFTWARE IS PROVIDED BY THE PROJECT AND CONTRIBUTORS \"AS IS\" AND\n");
    fprintf(f, "ANY EXPRESS OR IMPLIED WARRANTIES, INCLUDING, BUT NOT LIMITED TO, THE\n");
    fprintf(f, "IMPLIED WARRANTIES OF MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE\n");
    fprintf(f, "ARE DISCLAIMED.  IN NO EVENT SHALL THE PROJECT OR CONTRIBUTORS BE LIABLE\n");
    fprintf(f, "FOR ANY DIRECT, INDIRECT, INCIDENTAL, SPECIAL, EXEMPLARY, OR CONSEQUENTIAL\n");
    fprintf(f, "DAMAGES (INCLUDING, BUT NOT LIMITED TO, PROCUREMENT OF SUBSTITUTE GOODS\n");
    fprintf(f, "OR SERVICES; LOSS OF USE, DATA, OR PROFITS; OR BUSINESS INTERRUPTION)\n");
    fprintf(f, "HOWEVER CAUSED AND ON ANY THEORY OF LIABILITY, WHETHER IN CONTRACT, STRICT\n");
    fprintf(f, "LIABILITY, OR TORT (INCLUDING NEGLIGENCE OR OTHERWISE) ARISING IN ANY WAY\n");
    fprintf(f, "OUT OF THE USE OF THIS SOFTWARE, EVEN IF ADVISED OF THE POSSIBILITY OF\n");
...
usage(FILE *f)
{
    ...
    fprintf (f, " -t, --text-only        - enable ONLY text deltaQ output (no binary output)\n");
    fprintf (f, " -b, --binary-only      - enable ONLY binary deltaQ output (no text output)\n");
    fprintf (f, " -n, --no-deltaQ        - NO text NOR binary deltaQ output will be written\n");
    fprintf (f, " -m, --motion-nam       - enable output of motion data in NAM format\n");
    fprintf (f, " -s, --motion-ns        - enable output of motion data in NS-2 format\n");
    fprintf (f, " -j, --object           - enable output of object data\n");
    fprintf (f, " -o, --output <base>    - use <base> as base for generating output files,\n");
    fprintf (f, "                          instead of the input file name\n");
    fprintf (f, "Computation control:\n");
    fprintf (f, " -d, --disable-deltaQ   - disable deltaQ computation (output still generated)\n");
    fprintf (f, "\n");
    fprintf (f, "See the documentation for more usage details.\n");
    fprintf (f, "Please send any comments or bug reports to 'info@starbed.org'.\n\n");
}
...
static char *short_options = "hi:vltbnmsjo:d";
...
main (int argc, char **argv)
{
    ...
    struct io_connection_state_class *io_connection_state = NULL;
        ...
        switch (c) {
            ...
            case 'm':
                ...
                if (motion_output_enabled == TRUE) {
                    WARNING("Multiple arguments related to motion output detected.");
                    WARNING("Note that arguments '-m' and '-s' are mutually exclusive.");
                    usage(stdout);
                    exit(1);
                }
                else {
    ...
    if (xml_scenario == NULL) {
        ...
        WARNING("by editing the maximum data sizes constants in the file 'deltaQ/scenario.h'");
        WARNING("(MAX_NODES, MAX_OBJECTS, MAX_ENVIRONMENTS, MAX_MOTIONS and MAX_CONNECTIONS)");
        goto ERROR_HANDLE;
    }
    else {
        printf("MEMORY: Allocated %.2f MB (%zd bytes) for internal scenario storage.\n",
    ...
    // try to allocate the io_connection_state object
    io_connection_state = (io_cls *)malloc(sizeof (io_cls));

    if (io_connection_state == NULL) {
        WARNING("Cannot allocate memory (tried %zd bytes);", sizeof (io_cls));
        WARNING("you may need to reduce maximum scenario size");
        WARNING("by editing the maximum data sizes constants in the file 'deltaQ/scenario.h'");
        WARNING("(MAX_NODES, MAX_OBJECTS, MAX_ENVIRONMENTS, MAX_MOTIONS and MAX_CONNECTIONS)");
        ...
        printf("MEMORY: Allocated %.2f MB (%zd bytes) for connection state storage.\n",
                sizeof (io_cls) / 1048576.0, sizeof (io_cls));
    ...
    if (strlen(argv[optind]) > MAX_STRING) {
        WARNING("Input file name '%s' longer than %d characters!", argv[optind], MAX_STRING);
        goto ERROR_HANDLE;
    }
    else {
    ...
    DEBUG("Initialize random seed because it was changed by expat during parsing.\n");
    srand(1);
    ...
    if (!(text_only_enabled || binary_only_enabled)) {
        ...
        if (settings_file == NULL) {
            WARNING ("Cannot open settings file '%s' for writing!", settings_filename);
            goto ERROR_HANDLE;
        }
    }
    ...
    if (binary_output_enabled == TRUE) {
        ...
        strncat(binary_output_filename, ".bin", MAX_STRING - strlen(binary_output_filename) - 5);
        binary_output_file = fopen(binary_output_filename, "w");
        if (binary_output_file == NULL) {
            WARNING("Cannot open binary output file '%s' for writing!", binary_output_filename);
            goto ERROR_HANDLE;
        }
    ...
    if (motion_output_enabled == TRUE) {
        ...
        if (motion_output_type == MOTION_OUTPUT_NAM) {
            ...
            strncat(motion_filename, ".nam", MAX_STRING - strlen(motion_filename) - 5);
        }
        else if (motion_output_type == MOTION_OUTPUT_NS2) {
            // append extension ".ns2"
            strncat(motion_filename, ".ns2", MAX_STRING - strlen(motion_filename) - 5);
        }
        else {
        ...
        if (motion_file == NULL) {
            WARNING("Cannot open motion file '%s' for writing!", motion_filename);
            goto ERROR_HANDLE;
        }
    }

#ifdef MESSAGE_DEBUG
    int node_i, env_i, obj_i;
    ...
    if (object_output_enabled == TRUE) {
        ...
        strncat(object_output_filename, ".obj", MAX_STRING - strlen (object_output_filename) - 5);
        object_output_file = fopen (object_output_filename, "w");
        if (object_output_file == NULL) {
            WARNING("Cannot open object output file '%s' for writing!", object_output_filename);
            goto ERROR_HANDLE;
        }

        // write object output
        io_write_objects (scenario, xml_scenario->cartesian_coord_syst, object_output_file);
    ...
    for (current_time = xml_scenario->start_time;
        ...
        fprintf(stderr, "* Time=%.3f s: DONE", current_time);
        fprintf(stderr, "                                 \r");
        ...
        if (motion_output_enabled == TRUE) {
            if (current_time == xml_scenario->start_time) {
                ...
                if (motion_output_type == MOTION_OUTPUT_NAM) {
                    ...
                    io_write_ns2_motion_header_to_file_new(scenario, motion_file, current_time);
                }
            }
            else {
                // write motion info 
                if (motion_output_type == MOTION_OUTPUT_NAM)
                    io_write_nam_motion_info_to_file(scenario, motion_file, current_time);
                else
                    //io_write_ns2_motion_info_to_file (scenario, motion_file, current_time);
                    io_write_ns2_motion_info_to_file_new(scenario, motion_file, current_time);
            }
        }
        ...
        for (connection_i = 0; connection_i < scenario->connection_number; connection_i++) {
            if (own_id != -1) {
                if (scenario->connections[connection_i].from_node_index != own_id &&
                        scenario->connections[connection_i].to_node_index != own_id) {
                    continue;
                }
            }
            DEBUG("connections src id: %d, dst id: %d\n",
                    scenario->connections[connection_i].from_node_index,
                    scenario->connections[connection_i].to_node_index);

            struct connection_class *connection = &(scenario->connections[connection_i]);
            ...
            if (scenario->nodes[connection->from_node_index].interfaces[connection->from_interface_index].noise_source == TRUE) {
                continue;
            }
            ...
            if (binary_output_enabled == TRUE) {
                ...
                if (current_time == xml_scenario->start_time) {
                    ...
                    io_binary_build_record(&(io_connection_state->binary_records[connection_i]),
                    ...
                    if (io_binary_compare_record(&(io_connection_state->binary_records[connection_i]),
        ...
        if (binary_output_enabled == TRUE) {
            ...
            if (io_connection_state->binary_time_record.record_number > 0) {
                ...
                io_binary_write_time_record_to_file2(&(io_connection_state->binary_time_record), binary_output_file);
                ...
                for (connection_i = 0; connection_i < scenario->connection_number; connection_i++) {
                    ...
                    if (io_connection_state->state_changed[connection_i] == TRUE) {
                        io_binary_write_record_to_file2(&(io_connection_state->binary_records[connection_i]),
                             binary_output_file);
#ifdef MESSAGE_DEBUG
                        io_binary_print_record(&(io_connection_state->binary_records[connection_i]));
#endif
                ...
                if (record_i < io_connection_state->binary_time_record.record_number) {
                    WARNING ("At time %f wrote ONLY %d binary records out of %d",
                            current_time, record_i, io_connection_state->binary_time_record.record_number);
                }
                else {
                    INFO("At time %f wrote %d binary records", current_time, record_i);
                }
#ifdef DISABLE_EMPTY_TIME_RECORDS
            }
#endif
        ...
        for (divider_i = 0; divider_i < xml_scenario->motion_step_divider; divider_i++) {
            motion_current_time = current_time + divider_i * motion_step;

            if (motion_current_time > (xml_scenario->duration + xml_scenario->start_time + EPSILON)) {
                break;
            }
            ...
            motion_found = FALSE;
            for (motion_i = 0; motion_i < scenario->motion_number; motion_i++)
                if ((scenario->motions[motion_i].start_time <= motion_current_time) &&
                        (scenario->motions[motion_i].stop_time > motion_current_time)) {
                    fprintf (stderr, "\t\t\tcalculation => motion %d             \r", motion_i);
                    // TEMPORARY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (binary_output_enabled == TRUE) {
        ...
        io_binary_write_header_to_file(scenario->interface_number, time_record_number,
             MAJOR_VERSION, MINOR_VERSION, SUBMINOR_VERSION,
...
FINAL_HANDLE:
    ...
    if (error_status == SUCCESS) {
        ...
        INFO ("\n-- Scenario processing completed with errors (see the WARNING messages above)\n\n");
        fprintf (stderr, "\n\n-- Scenario processing completed with errors (see the WARNING messages above)\n\n");
    }
