...
sta_ap_send_beacon_routine (void *arg)
{
  ...
  // id and address of the BCAST socket used for sending traffic
  int bcast_out_socket_id = -1;
  struct sockaddr_in bcast_out_socket_addr;
  ...
  // Open a socket
  if ((bcast_out_socket_id = socket (PF_INET, SOCK_DGRAM, 0)) <= 0)
    {
      WARNING ("Station '%u': could not open BCAST outbound socket in send \
beacon thread.", station->id);
  ...
  if (setsockopt (bcast_out_socket_id, SOL_SOCKET, SO_BROADCAST,
		  &broadcastEnable, sizeof (broadcastEnable)) < 0)
    {
      WARNING ("Station '%u': error when setting socket option \
SO_BROADCAST", station->id);
      perror ("setsockopt");
      close (bcast_out_socket_id);
      return NULL;
    }

  // Configure the port and ip we want to send to
  memset (&bcast_out_socket_addr, 0, sizeof bcast_out_socket_addr);
  bcast_out_socket_addr.sin_family = AF_INET;

  // Set the broadcast IP address and the port (1900)
  inet_pton (AF_INET, BEACONS_ADDRESS, &bcast_out_socket_addr.sin_addr);
  bcast_out_socket_addr.sin_port = htons (BEACONS_PORT);
  ...
  while (station->do_interrupt == FALSE)
    {
      ...
      sent_byte_count
	=
	sendto (bcast_out_socket_id, &message, sizeof (struct message_class),
		0, (struct sockaddr *) &bcast_out_socket_addr,
		sizeof bcast_out_socket_addr);
...
{
  // id and address of the socket used for receiving traffic via BCAST
  int bcast_in_socket_id = -1;
  struct sockaddr_in bcast_in_socket_addr;
  ...
  /////////////////////////////////////////////////////
  // create broadcast outbound socket // FIXME: use unicast instead?!

  // create sending socket (SOCK_DGRAM signifies UDP, the only choice
  // available since we use MCAST)
  if ((bcast_out_socket_id = socket (PF_INET, SOCK_DGRAM, 0)) == -1)
    {
      WARNING ("Station '%u: could not open BCAST outbound socket in \
...
SO_BROADCAST", station->id);
  ...
  /*
     if (setsockopt (bcast_out_socket_id, IPPROTO_IP, IP_MULTICAST_IF,
  ...
  if ((bcast_in_socket_id = socket (PF_INET, SOCK_DGRAM, 0)) == -1)
    {
      WARNING ("Station '%u': could not create BCAST inbound socket in \
listen beacon thread", station->id);
      perror ("socket");
      return NULL;
    }
  ...
  // to the same socket
  if (setsockopt (bcast_in_socket_id, SOL_SOCKET, SO_REUSEADDR, &sockopt_val,
		  sizeof (sockopt_val)) < 0)
    {
      WARNING ("Station '%u': error when setting socket option SO_REUSEADDR",
	       station->id);
      perror ("setsockopt");
      close (bcast_in_socket_id);
      return NULL;
    }

  // configure MCAST socket address
  memset (&bcast_in_socket_addr, 0, sizeof (bcast_in_socket_addr));
  bcast_in_socket_addr.sin_family = AF_INET;
  //bcast_in_socket_addr.sin_addr.s_addr = INADDR_ANY;
  inet_pton (AF_INET, BEACONS_ADDRESS, &bcast_in_socket_addr.sin_addr);
  bcast_in_socket_addr.sin_port = htons (BEACONS_PORT);
  // ??? what are the lines below for?!
  //memset (mcast_in_socket_addr.sin_zero, '\0',
  //      sizeof (mcast_in_socket_addr.sin_zero));

  // binding to the appropriate port 
  if (bind (bcast_in_socket_id, (struct sockaddr *) &bcast_in_socket_addr,
	    sizeof (bcast_in_socket_addr)) == -1)
    {
      WARNING ("Station '%u': could not bind BCAST inbound socket in listen \
beacon thread.", station->id);
      perror ("bind");
      close (bcast_in_socket_id);
  ...
  while (station->do_interrupt == FALSE)
    {
      // receive from socket
      recv_byte_count =
	recvfrom (bcast_in_socket_id, &message, sizeof (struct message_class),
...
sending association request to AP '%s'", station->id, inet_ntoa (message.src_addr));
			  ...
			  sent_byte_count =
			    sendto (bcast_out_socket_id, &message,
				    sizeof (struct message_class), 0,
				    (struct sockaddr *)
				    &bcast_out_socket_addr,
				    sizeof (bcast_out_socket_addr));
...
from station '%s' => sending association response.", station->id, inet_ntoa (message.src_addr));
		  ...
		  sent_byte_count =
		    sendto (bcast_out_socket_id, &message,
			    sizeof (struct message_class), 0,
			    (struct sockaddr *) &bcast_out_socket_addr,
			    sizeof (bcast_out_socket_addr));
...
sta_init (struct station_class *station, int is_access_point,
	  unsigned id, struct in_addr *ip_address, unsigned beacon_interval)
{
